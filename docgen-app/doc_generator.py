...
# Matches a work-type code (T1..T5) at the start of a longer string like
# "T5 - Database to API" or "T5 (DB → API)". Ensures the checkbox row ticks
# the right box regardless of whether the LLM stored just the code or the label.
_WT_CODE_RE = re.compile(r'\bT([1-5])\b')


# ────────────────────────────────────────────────────────────────
#  SOP-driven standard defaults (baked in from the HL Architecture
#  SOP — SOP_Boomi_005). These populate fields that used to be TBD
#  when the requirement doesn't explicitly override them.
# ────────────────────────────────────────────────────────────────
SOP_DEFAULTS = {
    "transportSecurity": "TLS 1.2+ for HTTPS; SFTP over SSH; database connections encrypted in transit",
    "atRestSecurity": "Sensitive data in logs and staging encrypted at rest per data classification policy",
    "credentialManagement": "Credentials stored in a secrets vault (CyberArk / HashiCorp Vault / Boomi secure properties). Never hard-coded.",
    "piiHandling": "PII/PHI masked or tokenized where required; retention per organizational policy",
    "transientErrorHandling": "Exponential backoff with capped retries (configurable). Each retry attempt logged.",
    "permanentErrorHandling": "Payload moved to Dead Letter Queue (DLQ) with metadata; owner notified via configured channel.",
    "validationFailureHandling": "Record rejected with human-readable reason; sample payload persisted for analysis.",
    "alertingStrategy": "Critical errors trigger high-severity alerts to the on-call rotation; non-critical errors logged for periodic batch review.",
    "idempotencyStrategy": "Unique business keys (transaction ID, natural key) plus watermark on last processed position. Idempotency store for processed message IDs.",
    "loggingStandard": "Structured JSON logs with correlationId and messageId propagated end-to-end.",
    "monitoringStandard": "Metrics captured: success/failure counts, latency, retry counts, queue depth. Standard runbook attached.",
    "testingStrategy": "Unit (mapping/transformation), Integration (staging E2E), Contract (schema compatibility), Performance (SLA compliance), Security (vulnerability scan).",
    "deploymentControls": "CI/CD pipeline: DEV → QA → UAT → PROD. Version-controlled artifacts. Change request approval for production.",
    "governanceStandard": "Integration inventory maintained; periodic review of unused connectors and stale credentials.",
    "cicdPipeline": "Boomi packaged component + environment extensions per stage. Deployed via approved change ticket.",
    "runbookOwner": "Integration Operations / on-call rotation per COE runbook.",
}


# ────────────────────────────────────────────────────────────────
#  Middleware detection — Boomi vs Talend based on requirement
#  keywords. Talend SOP applies for S0-layer / Data Stewardship
#  patterns; Boomi is the default for everything else.
# ────────────────────────────────────────────────────────────────
_TALEND_KEYWORDS_RE = re.compile(
    r'\b(talend|s0\s*layer|data\s*stewardship|campaign\s*(creation|population)|talend\s*job)\b',
    re.IGNORECASE,
)


def detect_middleware(canonical):
    """Return 'Talend' or 'Dell Boomi' based on requirement content."""
    try:
        blob = json.dumps(canonical, default=str).lower()
    except Exception:
        blob = ""
    if _TALEND_KEYWORDS_RE.search(blob):
        return "Talend"
    return "Dell Boomi"


# ────────────────────────────────────────────────────────────────
#  SOP references — for the TDD "References" section. Maps
#  (middleware, workTypeCode) to a list of applicable SOPs.
# ────────────────────────────────────────────────────────────────
SOP_MASTER_REF = ("SOP_Boomi_005", "SOP for Any Source to Any Target Integration v1.0")

SOP_REFERENCES = {
    ("Dell Boomi", "T1"): [("SOP_Boomi_002", "SFTP → Database v1.0"), SOP_MASTER_REF],
    ("Dell Boomi", "T2"): [("SOP_Boomi_003", "API → Database v1.0"), SOP_MASTER_REF],
    ("Dell Boomi", "T3"): [("SOP_Boomi_004", "Database → Database v1.0"), SOP_MASTER_REF],
    ("Dell Boomi", "T4"): [SOP_MASTER_REF],
    ("Dell Boomi", "T5"): [("SOP_Boomi_001", "API → API v1.0 (target-side patterns apply)"), SOP_MASTER_REF],
    ("Talend",     "T1"): [("SOP_Talend_001", "Talend Source → S0 Integration v1.0"), SOP_MASTER_REF],
    ("Talend",     "T2"): [("SOP_Talend_001", "Talend Source → S0 Integration v1.0"), SOP_MASTER_REF],
    ("Talend",     "T3"): [("SOP_Talend_001", "Talend Source → S0 Integration v1.0"), SOP_MASTER_REF],
    ("Talend",     "T4"): [("SOP_Talend_001", "Talend Source → S0 Integration v1.0"), SOP_MASTER_REF],
    ("Talend",     "T5"): [("SOP_Talend_001", "Talend Source → S0 Integration v1.0"), SOP_MASTER_REF],
}


def sop_refs_for(middleware, wt_code):
    return SOP_REFERENCES.get((middleware, wt_code), [SOP_MASTER_REF])


# ────────────────────────────────────────────────────────────────
#  Procedural content per work-type (from the 5 procedural SOPs).
#  Each block gives the "Activities" list for the 5 standard SOP
#  steps. Fed into the TDD's process-oriented sections so the doc
#  matches the SOP for that type instead of generic prose.
# ────────────────────────────────────────────────────────────────
PROCEDURAL_STEPS_BY_TYPE = {
    "T1": {  # File → Database (SOP_Boomi_SFTP_to_Database)
        "requirementGathering": [
            "Identify source SFTP location, file format, delimiter, encoding, and target database.",
            "Collect field-level mapping and business rule documents.",
            "Confirm data frequency, expected file volume, and file naming convention.",
            "Review authentication method for SFTP (password / key-based) and DB (service account).",
            "Obtain stakeholder approval for file spec and mappings.",
        ],
        "connectivitySetup": [
            "Configure Boomi SFTP client connection and test file listing / retrieval.",
            "Configure Boomi Database connection and validate connectivity.",
            "Store SFTP credentials securely in Environment Extensions.",
            "Parameterize SFTP paths and DB connection strings per DEV / QA / PROD.",
        ],
        "processDevelopment": [
            "Design Boomi process: Start → SFTP Get → Map → Database (Insert/Upsert) → Stop.",
            "Create file profile matching the source layout.",
            "Apply transformations, cleansing rules, and business validations.",
            "Implement file archival / rename after successful processing.",
            "Add exception handling for missing / malformed files.",
            "Configure schedule per business frequency.",
        ],
        "testingValidation": [
            "Run in Test Atom Cloud with representative sample files.",
            "Validate file pickup, parsing, mapping, and load into DB.",
            "Verify record counts, field-level accuracy, and duplicate handling.",
            "Test archival and error-file quarantine.",
            "Obtain test sign-off from stakeholders.",
        ],
        "deploymentMonitoring": [
            "Create Change Request with deployment checklist attached.",
            "Deploy the process to the Production Atom (per governance).",
            "Verify environment extension values (SFTP path, credentials, DB connection).",
            "Monitor initial scheduled runs and validate DB load.",
            "Update the TDD with deployed version and CR reference.",
        ],
    },
    "T2": {  # API → Database (SOP_API_to_DB)
        "requirementGathering": [
            "Get API documentation from client/API tech team.",
            "Request API developer portal access if available.",
            "Set up a Postman collection for the API and validate before Boomi build.",
            "Collect field-level mapping and business rule documents.",
            "Confirm data frequency, volume, pagination style, and payload format (JSON / XML).",
            "Review authentication method (Bearer / OAuth2 / API Key).",
            "Obtain stakeholder approval for API specs and mappings.",
        ],
        "connectivitySetup": [
            "Configure HTTP Client connection and test with a sample GET call.",
            "Configure Boomi Database connection and validate.",
            "Store API credentials securely in Environment Extensions.",
            "Parameterize API base URLs, tokens, and DB connection strings per DEV / QA / PROD.",
        ],
        "processDevelopment": [
            "Design Boomi process: Start → HTTP Client → Map → Database → Stop.",
            "Create API response and DB table profiles; build the mapping.",
            "Apply transformations, paging logic, and business rules.",
            "Implement pagination (nextPageToken / offset / limit) if applicable.",
            "Implement exception handling for API failures and DB insert errors.",
            "Configure schedule or event trigger.",
        ],
        "testingValidation": [
            "Execute test runs in Boomi Test Atom Cloud.",
            "Validate API call, response parsing, transformation, and database load.",
            "Verify record counts, pagination behavior, and field-level accuracy.",
        ...
        "deploymentMonitoring": [
            "Create Change Request with deployment checklist attached.",
            "Deploy the process to the Production Atom (per governance).",
            "Verify process parameters, API base URLs, and DB extension values.",
            "Monitor initial runs and validate DB loads.",
            "Update the TDD with deployed version and CR reference.",
        ],
    },
    "T3": {  # Database → Database (SOP_DB_to_DB)
        "requirementGathering": [
            "Identify source and target databases, tables, and relationships.",
            "Collect field-level mapping and transformation rules.",
            "Confirm data frequency, volume, and change-detection strategy (CDC / timestamp).",
            "Review credentials and access for both databases.",
            "Obtain stakeholder approval for schema and mappings.",
        ],
        "connectivitySetup": [
            "Configure Boomi Database connections for source and target.",
            "Test SELECT on source and INSERT/UPDATE on target.",
            "Store credentials in Environment Extensions.",
            "Parameterize connection strings per DEV / QA / PROD.",
        ],
        "processDevelopment": [
            "Design Boomi process: Start → DB Get (source) → Map → DB Set (target) → Stop.",
            "Create source and target profiles; build mapping.",
            "Apply transformations, joins, and lookups.",
            "Implement watermark / last-run tracking for incremental pulls.",
            "Add exception handling for constraint violations and connection loss.",
        ],
        "testingValidation": [
            "Execute test runs in Boomi Test Atom Cloud.",
            "Validate row counts source vs target.",
            "Verify referential integrity, upsert logic, and edge cases.",
            "Document test results and resolve issues.",
            "Obtain test sign-off from stakeholders.",
        ],
        "deploymentMonitoring": [
            "Create Change Request with deployment checklist attached.",
            "Deploy to Production Atom (per governance).",
            "Verify DB connection extensions and watermark tables.",
            "Monitor initial runs and validate row counts.",
            "Update the TDD with deployed version and CR reference.",
        ],
    },
    "T4": {  # Database → File (uses master SOP)
        "requirementGathering": [
            "Identify source database and target file location / SFTP.",
            "Confirm file format (CSV / fixed-width / XML), delimiter, encoding, header row.",
            "Confirm data frequency, volume, and file naming convention.",
            "Obtain stakeholder approval for output file spec.",
        ],
        "connectivitySetup": [
            "Configure Boomi Database and SFTP connections.",
            "Store credentials in Environment Extensions.",
            "Parameterize output file path and connection strings per DEV / QA / PROD.",
        ],
        "processDevelopment": [
            "Design Boomi process: Start → DB Get → Map → File Write → SFTP Send → Stop.",
            "Build file profile matching the required layout.",
            "Apply transformations and business rules.",
            "Implement file naming with timestamp / sequence.",
            "Add exception handling for SFTP delivery failures.",
        ],
        "testingValidation": [
            "Execute test runs in Boomi Test Atom Cloud.",
            "Validate file content, row counts, and format against spec.",
            "Verify SFTP delivery and permissions.",
            "Obtain test sign-off from stakeholders.",
        ],
        "deploymentMonitoring": [
            "Create Change Request with deployment checklist attached.",
            "Deploy to Production Atom (per governance).",
            "Verify SFTP extension values and target directory.",
            "Monitor initial runs and confirm downstream consumption.",
            "Update the TDD with deployed version and CR reference.",
        ],
    },
    "T5": {  # Database → API (uses API→API SOP for target-side patterns)
        "requirementGathering": [
            "Identify source database and target API endpoints.",
            "Get target API documentation and set up a Postman collection.",
            "Collect field-level mapping and payload format requirements.",
            "Confirm data frequency, batch size, and payload format (JSON / XML).",
            "Review target API authentication (Bearer / OAuth2 / API Key).",
            "Obtain stakeholder approval for API specs and mappings.",
        ],
        "connectivitySetup": [
            "Configure Boomi Database (source) and HTTP Client (target) connections.",
            "Test with sample DB SELECT and API POST/PUT calls.",
            "Store API credentials securely in Environment Extensions.",
            "Parameterize base URLs, tokens, and DB connection strings per DEV / QA / PROD.",
        ],
        "processDevelopment": [
            "Design Boomi process: Start → DB Get → Map → HTTP Client → Stop.",
            "Create DB profile and API request/response profiles.",
            "Apply transformations and business rules.",
            "Implement batching or per-record POST as per API spec.",
            "Implement exception handling for API errors (4xx / 5xx) and DB issues.",
            "Configure schedule or event trigger.",
        ],
        "testingValidation": [
            "Execute test runs in Boomi Test Atom Cloud.",
            "Validate DB SELECT, transformation, and API call responses.",
            "Verify record counts, error handling, and idempotency.",
            "Document test results and resolve issues.",
            "Obtain test sign-off from stakeholders.",
        ],
        "deploymentMonitoring": [
            "Create Change Request with deployment checklist attached.",
            "Deploy to Production Atom (per governance).",
            "Verify API extension values (tokens, base URLs) and DB connection.",
            "Monitor initial runs and validate API delivery.",
            "Update the TDD with deployed version and CR reference.",
        ],
    },
}
...
def procedural_steps_for(wt_code):
    return PROCEDURAL_STEPS_BY_TYPE.get(wt_code, PROCEDURAL_STEPS_BY_TYPE["T2"])
...
def two_col_table(doc, rows_data, headers=None):
    """
    2-column label-value table.
    If headers=("Field","Value") is provided, a colored header row is added first
    and all data rows go below it via add_label_value_row.
    """
    if headers:
        table = doc.add_table(rows=1, cols=2)
        table.style = "Table Grid"
        make_header_row(table, list(headers))
        for i, (label, value) in enumerate(rows_data):
            add_label_value_row(table, label, value, shaded=(i % 2 == 1))
        return table

    # Legacy path — first row is data, no colored header
    table = doc.add_table(rows=1, cols=2)
    table.style = "Table Grid"
...
def generate_brd(c):
    ...
    _raw_wt = v_raw(c.get("integration", {}).get("workTypeCode"))
    _m = _WT_CODE_RE.search(_raw_wt)
    wt_code = f"T{_m.group(1)}" if _m else _raw_wt.strip()
    middleware = detect_middleware(c)
    ...
    # Version table — proper 2-col label/value with a colored header
    two_col_table(doc, [
    ...
    two_col_table(doc, [
        ...
        ["Visual Flow Diagrams",   f"See attached Flow_{re.sub(r'[^A-Za-z0-9_]+', '_', project_name)[:50]}_v1_0.drawio"],
...
def generate_tdd(c):
    ...
    _m = _WT_CODE_RE.search(_raw_wt)
    wt_code = f"T{_m.group(1)}" if _m else _raw_wt.strip()
    steps = procedural_steps_for(wt_code)
    # ^ dict keyed by phase name — held under `sop_steps` for clarity below
    sop_steps = steps

    # ── Title Page ──
    title = doc.add_paragraph()
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = title.add_run(f"Enterprise {middleware} Technical Design Document (TDD)")
    run.bold = True
    run.font.size = Pt(16)
    run.font.color.rgb = RGBColor.from_string(HEADER_COLOR)
    ...
    # (Section 3 "Assumptions" removed per SOP feedback — assumptions
    # covered under the SOP standard patterns / project stakeholders.)
    ...
    add_bullets(doc, [
        SOP_DEFAULTS["loggingStandard"] + " [STANDARD_DEFAULT]",
        "Process Name, Execution ID, Start / End Time, records processed / loaded / failed",
        "Error details for failed records",
    ])
    add_heading(doc, "Monitoring Requirements", level=2)
    add_bullets(doc, [
        SOP_DEFAULTS["monitoringStandard"] + " [STANDARD_DEFAULT]",
        "Success rate, failure rate, throughput, average execution time",
    ])

    # ── Section 21: Notifications ──
    add_heading(doc, "21. Notification Strategy", level=1)
    add_heading(doc, "Success", level=2)
    add_para(doc, "Recipients: TBD (project-specific)\nBatch completed successfully. Records loaded: <count>.")
    add_heading(doc, "Failure", level=2)
    add_para(doc, "Recipients: TBD (project-specific)\n" + SOP_DEFAULTS["alertingStrategy"] + " [STANDARD_DEFAULT]")
    ...
    add_bullets(doc, [
        ...
        SOP_DEFAULTS["transportSecurity"] + " [STANDARD_DEFAULT]",
        SOP_DEFAULTS["atRestSecurity"] + " [STANDARD_DEFAULT]",
        SOP_DEFAULTS["credentialManagement"] + " [STANDARD_DEFAULT]",
        SOP_DEFAULTS["piiHandling"] + " [STANDARD_DEFAULT]",
    ])

    # ── Section 23: Retention ──
    add_heading(doc, "23. Archive and Retention", level=1)
    two_col_table(doc, [
        ["Success Records", "Per data classification policy [STANDARD_DEFAULT]"],
        ["Error Records",   "Retained per organizational retention policy [STANDARD_DEFAULT]"],
        ["Logs",            "Retained per organizational retention policy [STANDARD_DEFAULT]"],
    ...
    add_para(doc, SOP_DEFAULTS["testingStrategy"] + " [STANDARD_DEFAULT]")
    ...
    add_para(doc, SOP_DEFAULTS["deploymentControls"] + " [STANDARD_DEFAULT]")
    add_heading(doc, "Pre-Deployment Checklist", level=2)
    add_bullets(doc, sop_steps["deploymentMonitoring"])
    add_heading(doc, "Deployment Steps", level=2)
    add_bullets(doc, [
        f"Package deployment to target {middleware} environment",
    ...
    add_heading(doc, "Applicable SOPs", level=2)
    sop_bullets = []
    for sop_id, sop_name in sop_refs_for(middleware, wt_code):
        sop_bullets.append(f"{sop_id} — {sop_name}")
    add_bullets(doc, sop_bullets)
    add_heading(doc, "Supporting Documents", level=2)
    add_bullets(doc, [
        f"BRD — {project_name}",
        "Field Mapping Specification (attached separately)",
        "Source System Documentation (project-specific)",
        "Target System Documentation (project-specific)",
    ])
    ...
    # (Unresolved Items section removed per SOP feedback.)
